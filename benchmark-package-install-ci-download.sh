...
hyperfine \
  ...
  --command-name='Yarn (website)' --prepare='rm -rf ~/.cache/yarn/ website-node-20/node_modules/' 'cd website-node-20 && yarn install --immutable' \
...
hyperfine \
  ...
  --warmup=1 \
  --time-unit=millisecond \
  --command-name='Yarn (analytics)' --prepare='rm -rf ~/.cache/yarn/ website-node-20/analytics/node_modules/' 'cd website-node-20/analytics && yarn install --immutable' \
