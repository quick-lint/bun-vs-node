...
hyperfine \
  ...
  --prepare='cd website-bunbuns && bun install && ../start-server.sh bun run run-server-worker.mjs' 'cd website-bunbuns && bun run tools/check-links.mjs http://localhost:9001/' \
